...
def all_supplies_in_holidays(holiday_hash)
  ...
  holiday_hash.each do |season, data|
    ...
    data.each do |holiday_name, supplies|
      holiday_string = "  "
      holiday_name_arr = holiday_name.to_s.split("_")
      holiday_name_arr_capitalize =
        holiday_name_arr.collect do |token|
          token.capitalize!
        end
      holiday_string << holiday_name_arr_capitalize.join(" ")
      holiday_string << ":"
      first_supply = 1
...
def all_holidays_with_bbq(holiday_hash)
  ...
  holiday_hash.each do |season, season_data|
    season_data.each do |holiday_name, holiday_supplies|
